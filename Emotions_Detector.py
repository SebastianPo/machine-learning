...
def separate_data(happy_images, sad_images):
    ...
    test_images = []
    test_labels = []
    how_many_images_to_test=12

    for i in range(how_many_images_to_test):
        test_images.append(happy_images[i])
        test_labels.append([1, 0])
        test_images.append(sad_images[i])
        test_labels.append([0, 1])

    for i in range(len(happy_images) - how_many_images_to_test):
        train_images.append(happy_images[i + how_many_images_to_test])
        train_labels.append([1, 0])
        train_images.append(sad_images[i + how_many_images_to_test])
...
def create_model():
    ...
    model.add(
        Conv2D(32, (2, 2), input_shape=(3, 32, 32), padding='same', activation='relu', kernel_constraint=maxnorm(3)))
...
def train_and_assess_model(model, train_images, train_labels):
    ...
    sgd = SGD(lr=learning_rate, momentum=0.81, decay=decay, nesterov=False)
    ...
    tf.train.write_graph(frozen_graph, '.', 'Android_App/app/src/main/assets/emotions_detector.pb', as_text=False)
